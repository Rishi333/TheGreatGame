...
public class GlLibrary {
    ...
    public static void checkLag(long time, int frameRate){

        long timeTaken= System.currentTimeMillis()-time;
        if(timeTaken<1000/frameRate)
        {
            try {
                Thread.sleep(1000/frameRate-timeTaken);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }else{
            System.out.println("Time Warning, Frame exceeded by : " + ((int)timeTaken-(1000/frameRate))+" milliseconds");
            try {
                Thread.sleep(20);
            } catch (InterruptedException e) {
                e.printStackTrace();
    ...
    public static void glDraw(int tex ,int x, int y, int w, int h,GL2 gl) {
